...
public interface DailyBatchExecutionMapper {
    ...
    void insert(DailyBatchExecution execution);

    /**
     * 만료됐거나(status != 'RUNNING' 또는 lease_until 경과) 완료된 기존 실행을 새 lease로 인계한다.
     * 영향받은 row 수가 1이어야 실제로 lease를 획득한 것이다.
     * "현재 시각"은 DB의 {@code NOW()}가 아니라 애플리케이션 Clock으로 계산해 넘긴다.
     * DB 세션 타임존이 애플리케이션 Clock(Asia/Seoul)과 다를 수 있어(예: 컨테이너 기본값 UTC),
     * {@code NOW()}에 의존하면 실제로는 만료되지 않은 lease를 만료로 잘못 판단할 수 있다.
     */
    int acquireExpiredLease(@Param("execution") DailyBatchExecution execution, @Param("now") LocalDateTime now);
    ...
    int renewLease(@Param("id") Long id, @Param("ownerId") String ownerId,
                   @Param("leaseToken") String leaseToken, @Param("newLeaseUntil") LocalDateTime newLeaseUntil,
