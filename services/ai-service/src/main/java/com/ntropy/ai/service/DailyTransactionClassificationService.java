...
public class DailyTransactionClassificationService implements TransactionClassificationCommandClient {
    ...
    public DailyTransactionClassificationService(
            AccountTransactionAnalysisClient accountTransactionAnalysisClient,
        ...
        this.accountTransactionAnalysisClient = accountTransactionAnalysisClient;
    ...
    private List<TransactionAnalysisSaveItem> classifyPage(
         ...
         * 최대 100건씩 나눠서 호출합니다.
         */
        List<CompletableFuture<List<TransactionAnalysisSaveItem>>> futures =
                new ArrayList<>();

        int batchNumber = 0;
        for (int start = 0; start < fastApiTargets.size(); start += FAST_API_BATCH_SIZE) {
            int end = Math.min(
                    start + FAST_API_BATCH_SIZE,
                    fastApiTargets.size()
            );

            List<DailyClassificationTargetTransaction> batch =
        ...
        for (CompletableFuture<List<TransactionAnalysisSaveItem>> future : futures) {
    ...
    private List<TransactionAnalysisSaveItem> classifyWithFastApi(
         ...
         */
        for (DailyClassificationTargetTransaction target : targets) {
            TransactionAnalysisSaveItem result = validResults.get(target.getTransactionId());
            if (result == null) {
                fallbackCount++;
                result = fallback(target.getTransactionId());
            }
